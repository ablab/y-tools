...
namespace SequenceMerger {
    ...
    std::string SequenceMerger::reverse_quality_string(std::string qual) {
    ...
    std::pair<size_t, size_t> SequenceMerger::FindBestOverlap(const std::string& seq1, const std::string& seq2) {
    ...
    std::pair<std::string, std::string> SequenceMerger::MergeQualifiedSeq(const PairedFastqRead& paired_read) {
    ...
    std::string SequenceMerger::MergeNames(size_t index, const std::string& name1, const std::string&) {
        const std::string name = (name1[0] == '@') ? name1.substr(1, name1.size() - 1) : name1;
        std::stringstream ss;
        ss << "@" << index << "_merged_read_" << delete_whitespaces(name);
        return ss.str();
    }

    FastqRead SequenceMerger::Merge(size_t index, const PairedFastqRead& paired_read) {
        std::string merged_name = MergeNames(index, paired_read.left_read.name, paired_read.right_read.name);
        std::pair<std::string, std::string> merged_seq_qual = MergeQualifiedSeq(paired_read);
        return FastqRead(merged_name, merged_seq_qual.first, merged_seq_qual.second);
    }

    std::vector<FastqRead> PairedReadsMerger::Merge(const std::vector<PairedFastqRead>& reads) {
