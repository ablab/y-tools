...
namespace antevolo {
    ...
    size_t CDR3HammingGraphComponentInfo::GetOldIndexByCDR3(const std::string& cdr3) {
        VERIFY_MSG(cdr3_to_old_index_map_.find(cdr3) != cdr3_to_old_index_map_.end(),
                   "CDR3HammingGraphComponentInfo: failed to find cdr3 " << cdr3);
        return cdr3_to_old_index_map_.find(cdr3)->second;
    }

    size_t CDR3HammingGraphComponentInfo::GetOldIndexByCDR3JNucl(std::string cdr3JNucl) {
        VERIFY_MSG(cdr3_to_old_index_map_.find(cdr3JNucl) != cdr3_to_old_index_map_.end(),
                   "CDR3HammingGraphComponentInfo: failed to find cdr3JNucl");
